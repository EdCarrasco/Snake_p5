function Snake() {
	...
	this.update = function() {
		...
		this.x = constrain(this.x, gameScale, width - gameScale);
		this.y = constrain(this.y, gameScale, height - gameScale);
	...
	this.show = function() {
		...
		if (this.decayRate > 0) {
			...
			for (var i = 0; i < this.corpse.length; i++) {
				...
				rect(this.corpse[i].x, this.corpse[i].y, size, size);
			}
			this.decayRate -= 3;
		}
	}
